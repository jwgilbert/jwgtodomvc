$(function() {
	...
	function standUp() {
		...
		if(localStorage && localStorage.length > 0) {
			...
			// console.log("There are no entries in localStorage");
		}
	...
	function insertEntry(entry, id, status) {
		...
		$('.template li').clone().appendTo('#todoItems');
		$('#todoItems li:last-child label').text(entry);
		$('#todoItems li:last-child').attr('data-id',id);
		// $('#inputBox').val('');
		if(status) {
			$('todoItems li:last-child').addClass('completed');
			$('todoItems li:last-child .toggle').attr('checked', true)
		}
		$('#inputBox').val('');
